...
class change_joint_velocity_limit(ManagerTermBase):
    def __init__(self, cfg: EventTermCfg, env: ManagerBasedEnv):
        super().__init__(cfg, env)
        # extract the used quantities (to enable type-hinting)
        self.asset_cfg: SceneEntityCfg = cfg.params["asset_cfg"]
        ...
        if isinstance(self.asset, Articulation) and self.asset_cfg.body_ids != slice(None):
            ...
            if num_shapes != expected_shapes:
                raise ValueError(
                    "Joint setting change term 'change_joint_velocity_limit' failed to parse the number of shapes per body."
                    f" Expected total shapes: {expected_shapes}, but got: {num_shapes}."
                )
        else:
            # in this case, we don't need to do special indexing
    ...
    def __call__(
        ...
        if env_ids is None:
            ...
            env_ids = env_ids.cuda()
        joint_velocity_torch = torch.tensor(joint_velocity,device="cuda")
        # apply to simulation
        self.asset.write_joint_velocity_limit_to_sim(joint_velocity_torch,asset_cfg.joint_ids, env_ids)