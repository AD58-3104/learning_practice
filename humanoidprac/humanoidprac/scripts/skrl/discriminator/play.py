...
def main():
    ...
    memory = RandomMemory(
        memory_size=experiment_cfg["memory"]["memory_size"],
    ...
    experiment_cfg["agent"]["learning_rate_scheduler"] = KLAdaptiveRL

    from skrl.agents.torch.ppo import PPO
    agent = PPO(
        models=models,  # models dict
        memory=memory,  # memory instance, or None if not required
        cfg=experiment_cfg["agent"],  # configuration dict (preprocessors, learning rate schedulers, etc.)
        observation_space=env.observation_space,
